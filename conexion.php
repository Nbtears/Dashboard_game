<?php 
    class Connection
    {
        ...
        public function all_games($loginuser,$conn){
            $sql = "SELECT angle_max,angle_avg, rep_flex, rep_sup, vel_max, acc_max, vel_avg, acc_avg,score, DATE(sesion.date) as fecha FROM game, sesion WHERE game.sesion = sesion.id AND Sesion.user = '" . $loginuser."'";
            $result = $conn ->query($sql);
            $row = array();
